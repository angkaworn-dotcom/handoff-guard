...
อยากได้ `handoff` เวอร์ชันล่าสุดจาก upstream: `node ~/.claude/skills/handoff-guard/scripts/ensure-handoff.mjs --check` ดู diff ก่อน แล้ว `--update` เพื่อรับมา (สำรองตัวเก่าเป็น `SKILL.md.bak` ให้ · เสร็จแล้ว restart session) — การอัปเดตเป็นการสั่งเองเสมอ ไม่ดึงอัตโนมัติ
