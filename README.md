...
- ⚠️ **ใกล้เต็ม** — ถึง 85% ของเพดาน
- 🔴 **ด่วน** — ถึง 94% ของเพดาน

มันรู้เพดานของแต่ละ model เอง (Opus 256k, Sonnet/Haiku 200k) และถ้า session โดน compact ไปแล้วโตกลับมาใกล้เต็มอีก มันจะเตือนซ้ำได้
...
```bash
node ~/.claude/skills/handoff-guard/scripts/selftest.mjs   # ต้องขึ้น ALL PASS (32 เคส)
```
...
**ง่ายสุด** พิมพ์ในแชท `/handoff-guard-max 200000` — ตั้งเพดานทันที มีผลเทิร์นถัดไปไม่ต้อง restart (`/handoff-guard-max reset` กลับไป auto)

หรือตั้งผ่าน env (env ชนะทุกอย่างเสมอ เหมาะกับ override ชั่วคราว/ทดสอบ):

| env | ค่า default | ความหมาย |
|-----|-------------|----------|
| `HANDOFF_GUARD_MAX` | auto ตาม model | เพดาน context — Opus 256k, Sonnet/Haiku/ไม่รู้จัก 200k |
| `HANDOFF_GUARD_THRESHOLD` | 85% ของเพดาน | ระดับ "ใกล้เต็ม" |
| `HANDOFF_GUARD_THRESHOLD2` | 94% ของเพดาน | ระดับ "ด่วน" |
