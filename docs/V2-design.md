...
> อัปเกรด `handoff-guard` จาก **reactive** (รอถึง 170k ค่อยทำ) เป็น **predictive** (คาดว่าอีก ~N เทิร์นจะแตะเขตอันตราย → เตรียม handoff ตั้งแต่ตอนนี้) โดยคงกลไกเดิมทั้งหมดเป็น **safety net**
>
...
```jsonc
{
  "lastTokens": 152340,  // token รอบก่อน (ไว้คำนวณ delta)
...
```
1. tokens ≥ T2 (188k) & !marker.t2   → fire "tier2"   (ด่วน — เดิม)
2. tokens ≥ T1 (170k) & !marker.t1   → fire "tier1"   (เดิม)
...
> 🟡 คาดการณ์: context ~152k, โตเฉลี่ย ~8.2k/เทิร์น → อีก ~3 เทิร์นจะแตะ 188k. ปิด step ปัจจุบันให้จบ แล้ว invoke skill "handoff-guard" เพื่อเตรียม handoff. อย่าเริ่มงานใหญ่ใหม่.
...
| สัญญาณ | ตัดสิน |
|---|---|
| **predict tier (token ยังไม่ถึง 170k, buffer เยอะ)** | **ปิด step ปัจจุบันให้จบสวยๆ ได้** แล้วค่อย handoff · ห้ามเริ่ม feature/refactor ใหม่ |
| tier1 (≥170k) ... | (เดิม) |
| tier2 (≥188k) ... | (เดิม) |
...
|---|---|---|
| `HANDOFF_GUARD_THRESHOLD` | 170000 | T1 (absolute tier1 — เดิม) |
| `HANDOFF_GUARD_THRESHOLD2` | 188000 | T2 (absolute tier2 + เป้าของ ETA — เดิม) |
| `HANDOFF_GUARD_PREDICT_TURNS` | 3 | K — lead time (เทิร์น) ของ predict trigger (ใหม่) |
| `HANDOFF_GUARD_EMA_ALPHA` | 0.4 | น้ำหนัก EWMA (ใหม่) |
...
1. โต 8k/เทิร์นสม่ำเสมอ จาก 150k → predict ยิงตอน ETA ≤ 3 (≈164k) ก่อนถึง 170k
...
5. absolute tier เดิม: 169k ไม่ยิง / 171k ยิง tier1 / 188k ยิง tier2 (regression — ต้องคงผ่าน)
