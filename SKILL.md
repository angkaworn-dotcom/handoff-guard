...
- **เพดาน adapt ตามโมเดล** — MAX auto-detect จาก `message.model` (opus 256k · sonnet/haiku 200k) → ยิงตรงเพดานจริงของแต่ละโมเดล ไม่ใช่ค่าตายตัว (สลับโมเดลกลางเซสชันได้) · pin เองได้ด้วย `/handoff-guard-max`
