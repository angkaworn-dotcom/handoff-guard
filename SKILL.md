...
## เมื่อไหร่ถูกเรียก
> T1/T2 = `round(MAX×0.72)` / `round(MAX×0.85)` · MAX **auto-detect ตามโมเดล** (fable/opus 256k → T1≈184k/T2≈218k · sonnet/haiku 200k → T1=144k/T2=170k · `[1m]` 1M) หรือ pin เองด้วย `/handoff-guard-max`
...
- **เพดาน adapt ตามโมเดล** — MAX auto-detect จาก `message.model` (fable/opus 256k · sonnet/haiku 200k · `[1m]` 1M) → ยิงตรงเพดานจริงของแต่ละโมเดล ไม่ใช่ค่าตายตัว (สลับโมเดลกลางเซสชันได้) · pin เองได้ด้วย `/handoff-guard-max`
