...
```
1. tokens ≥ T2 (240k) & !marker.t2   → fire "tier2"   (ด่วน — เดิม)
2. tokens ≥ T1 (218k) & !marker.t1   → fire "tier1"   (เดิม)
...
> 🟡 คาดการณ์: context ~216k, โตเฉลี่ย ~8k/เทิร์น → อีก ~3 เทิร์นจะแตะ 240k. ปิด step ปัจจุบันให้จบ แล้ว invoke skill "handoff-guard" เพื่อเตรียม handoff. อย่าเริ่มงานใหญ่ใหม่.
...
| สัญญาณ | ตัดสิน |
|---|---|
| **predict tier (token ยังไม่ถึง 218k, buffer เยอะ)** | **ปิด step ปัจจุบันให้จบสวยๆ ได้** แล้วค่อย handoff · ห้ามเริ่ม feature/refactor ใหม่ |
| tier1 (≥218k) ... | (เดิม) |
| tier2 (≥240k) ... | (เดิม) |
...
|---|---|---|
| `HANDOFF_GUARD_THRESHOLD` | 218000 | T1 (absolute tier1) = 85% ของเพดาน 256k |
| `HANDOFF_GUARD_THRESHOLD2` | 240000 | T2 (absolute tier2 + เป้าของ ETA) = 94% ของเพดาน 256k |
...
> **เพดาน 256k** — T1/T2 ตั้งไว้ 85%/94% ของเพดาน (ของเดิมเคยอิง 200k = 170k/188k) · ถ้าเปลี่ยนเพดานในอนาคต คำนวณ T1=ceil(MAX×0.85), T2=ceil(MAX×0.94)
...
1. โต 8k/เทิร์นสม่ำเสมอ จาก 200k → predict ยิงตอน ETA ≤ 3 (≈216k) ก่อนถึง 218k
...
5. absolute tier เดิม: 217k ไม่ยิง / 219k ยิง tier1 / 241k ยิง tier2 (regression — ต้องคงผ่าน)
