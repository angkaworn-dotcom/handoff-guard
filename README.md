...
**Dependency:** ต้องมี skill `handoff` (superpowers/Matt) ติดตั้งด้วย — guard นี้ **บังคับใช้** มันสร้าง handoff doc (คุณภาพดีกว่า: เซฟ temp ไม่รก repo, suggested-skills, เลี่ยง duplicate, redact secret) · มี fail-safe เขียน `HANDOFF.md` สั้นๆ กันงานหายถ้า `handoff` ยังไม่ได้ติดตั้ง · นอกนั้นพึ่งแค่ `node` บน PATH
...
```bash
# 0) prerequisite: ติดตั้ง skill `handoff` (superpowers/Matt) ที่ ~/.claude/skills/handoff/  ← guard บังคับใช้
# 1) skill
